...
static double 
removeTest(std::set<Foo*> &foo_ptrs, const std::vector<Foo*> &to_remove)
{
  namespace c = std::chrono;
  auto start = c::high_resolution_clock::now();
...
static double 
addTest(std::set<Foo*> &foo_set, const std::vector<Foo*> &to_add)
{
  namespace c = std::chrono;
  auto start = c::high_resolution_clock::now();
...
TestResult 
setTest(int iters, int num_elements, int num_add_remove)
{
  using std::vector;
  using std::set;
